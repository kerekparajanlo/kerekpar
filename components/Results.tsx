...
export default function Results({ bike, formData }: ResultsProps) {
  const dealers = dealersList.filter(d => normalizeCity(d.city) === normalizeCity(formData.city));

  useEffect(() => {
    const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
    const key = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

    console.log('[Supabase] URL prefix:', url?.substring(0, 30) ?? 'UNDEFINED');
    console.log('[Supabase] Key prefix:', key?.substring(0, 10) ?? 'UNDEFINED');
